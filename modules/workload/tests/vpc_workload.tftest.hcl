...
run "plans_private_ipam_vpc_with_two_azs" {
  ...
  assert {
    condition     = length(aws_subnet.transit_gateway_attachment) == length(var.transit_gateway_attachment_subnets)
    error_message = "A Transit Gateway attachment must use dedicated transit subnets, never application private subnets."
  }

  assert {
    condition     = length(aws_route.private_to_transit_gateway) == length(var.availability_zones)
    error_message = "Every application private route table must receive each explicitly approved Transit Gateway route."
  }
}
...
run "rejects_default_route_to_transit_gateway" {
  command = plan
  ...
  expect_failures = [var.transit_gateway_routes]
}
